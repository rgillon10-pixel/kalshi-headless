...
**The milestone (Q42 part-1 residual).** Delegated to `research-lead` → `edge-prober`, which built `scripts/q42_funding_estimate_path_inference.py` (+87 offline tests, no network) joining Kalshi's own published forward `funding_estimate` path to finalized funding prints — 286 joined windows across 13 tickers / 22 funding_times, both `funding_rates` modes (`backfill`/`recent`) deduped per L137, avoiding the nested-`prints[]` schema trap that silently returns 0 rows on a naive top-level join.

**Self-correction, twice, both caught by an independent `verifier` (the two-agent rule earning its keep).** The producer's first-pass verdict, "H1 FALSIFIED" (no `g`/θ reproduces the venue's clamp), was **REFUTED**: the pooled "no hard gap" result turned out to be a capture-staleness artifact (the `g_last` gap is monotone in path density and turns into a *hard* gap on denser subsets — automatic for nested subsets, so on its own this is a tautology, not evidence), the stated data-adequacy argument ("denser data can only add windows, not remove ones already in the gap") was logically false for a statistic computed from a *sampled path*, and the headline Fisher-exact p (5.30e-10) was pseudo-replicated (286 windows are 13 tickers × 22 times, not 286 independent draws — L6). Corrected verdict: **H1 UNDECIDABLE at current tape density.** A second verifier round then caught and fixed 5 further issues in the correction itself: a leave-one-out drop-count arithmetic error (13+22+42 summed to 77, not the stated 67; the discriminating population's real span is 7 tickers + 18 funding_times + 42 windows = 67), a "leave-one-out passes" claim that was true but existed only in a throwaway session rather than the shipped script (now `leave_one_out_gap_scan()`, tape-pinned), the gap-monotonicity flag mislabeled as informative when it is mathematically guaranteed on any nested chain (replaced with a matched-size random-subset baseline: a hard gap on a random same-size cut occurs 10-21% of the time, so neither dense stratum beats chance), an over-counted "~3 of 13 tickers carry the association" claim (correct count: 2 — ZEC and SUI; HYPE's borderline p is a single degenerate window), and an unquantified post-hoc-cuts multiplicity correction (9 cuts searched; Bonferroni leaves both candidate strata non-significant at 0.05). The verdict sentence itself never moved across either round — only the supporting numbers did, and the file keeps both correction sections in place rather than rewriting history.

**Durable result, independent of density.** Before any `g` (summary statistic of the estimate path) can be tested, the regressor itself has to be checked: **1,274 published estimate samples, 93.49% exactly zero, 0 nonzeros in the open interval (0, 1e-4)** — the same hard-gap shape part 1 found on the *finalized* prints. Kalshi clamps the forward estimate it publishes, not just the settlement print. Consequence: the pre-clamp quantity the settlement formula presumably acts on may never be observable from this endpoint at *any* capture density — a stronger and more durable caution than the threshold-leg result. Every funding number in this milestone (`broker_truth`) is a venue-computed value, never a fillable price; no P&L claim, no bootstrap CI, no registry change. Findings: `findings/2026-07-24-q42-funding-estimate-path-inference.md`.

**Idle unit run in parallel — L136 → GATING ratchet.** `scripts/invariants.py` gained `inv_no_raw_datetime_fromisoformat`: a static ratchet (`LEGACY_RAW_FROMISOFORMAT_SITES`, 33 files / 39 sites pinned at today's count, pins may only shrink) that fails the `--full` gate on any NEW raw `datetime.fromisoformat(` call site outside the sanctioned `core/timeutil.py`. New lesson **L151**: this has to be a static assert rather than a test, because the hazard (Python 3.9 rejecting bare-`Z`/short-fraction timestamps) is invisible to a suite that only ever runs under this sandbox's 3.11 — every hazardous shape parses fine there, so a passing test suite was never evidence of safety. Also corrected the stale `UNENFORCED` status on **L74**, **L109**, and **L123** — each one's proposed enforcement had already been built by a later run, but the status column was never updated (lesson text left untouched, same precedent as L25's 2026-07-21 correction); L109 specifically cost this run's lead a false start (it picked L109 for enforcement, then discovered the orphan-classifier already existed) — which is exactly what **L152** now names as its own failure class.

**Gates.** `pytest`: 1541 → **1665 passed**, 0 failed (run twice independently by the orchestrating session after the sub-agent's own report, both exit 0). `python scripts/invariants.py --full`: exit 0 — same advisory classes as before, except the raw-`fromisoformat` count is now GATED (not merely advisory) via the new ratchet.

**Step 9 (paper sub-pass).** `SHADOW_REGISTRY={s14_ladder_underwriting}` (DEAD per Q34 — paper-infra validation only, NOT edge evidence). `scripts/paper_pass.py`: 0 newly-eligible events this pass, ledger unchanged, realized P&L **+$15.69** (`broker_truth`). Still **0 proven edges**.

---

## 2026-07-24 ~12:30 UTC — IDLE RUN (policy c): committed-tape timestamp-parseability audit — the Python-3.9 `fromisoformat` hazard (L136/L138) is LIVE at 38% (research loop)

**Step 0/0a/0b + rescan.** History-integrity PASS (origin/main HEAD `ce8989b`, kb/00-LOG newest 07-24 ~09-10, tape newest dt=2026-07-24). Full independent Q0-Q47 rescan → queue SATURATED (idle run), re-verifying the live gates myself against committed tape rather than trusting prior runs: **Q19** FOMC gate opens 2026-07-29 (today 07-24 — not open); **Q36** Part-1 still **2/10** KXTEMPNYCH settled events in `tape/settlement_ledger/` (`KXTEMPNYCH-26JUL1707`, `-26JUL2205`; MIN_EVENTS=10 — still frozen) and Part-2 structurally capped at ~1 book/market-hour; **Q43** density unchanged/thinner (`crypto_hourly/dt=2026-07-24`=6 lines, `perp_tape/dt=2026-07-24`=34 lines / 2 captures) — the same data-inadequate state the 07-23/24 probe runs already hit. Q21 idea-gen round was already completed today (07-24) by the edge-hunter (S48/S49/S50 all killed → next free S51). Confirms the #172-186 "0 eligible, saturated" conclusion independently.

**Idle-run policy selection.** (a) genuinely exhausted — every fresh UNENFORCED lesson is already converted (L141/L144/L147/L148/L149 in the last two days); the remaining UNENFORCED-marked rows are stale markers (L45/L47/L59/L64/L76/L86/L90/L105 all already superseded/encoded — L105's edge-prober bullet already exists at lines 188-207), Ryan/policy-gated (L22 real_bid, L145 ws_depth), network-dependent (L17), or terminal-as-protocol-and-already-encoded (L27/L28/L32/L34/L39/L51/L69). (b) all time-gated probes (Q19 FOMC burst-mode, Q36 settlement-basis, Q37 summer maker-NO, Q43 perp-consistency) are already prepped/self-activating. (d) just ran today. → took **(c)**, a data-quality deep-dive on a genuinely FRESH dimension not covered by L142 (conflict markers) / L143 (zero/null fields) / L148 (JSON structural validity) / Q43 (record_type) / #178/#180 (universe_sweep breadth).

**The milestone.** L138's own row said the literal Python-3.9 `datetime.fromisoformat` failure "could not be proven against the literal 3.9 failure here" (this sandbox is 3.11). Never-measured question: does committed TAPE actually carry the hazardous shape, or is the L138/L141 raw-`fromisoformat` migration backlog (36 code sites) theoretical? Built `scripts/tape_timestamp_parseability_audit.py` (+12 offline tests) — read-only, offline, emulates 3.9 by a regex/digit-count rule (bare trailing `Z` OR fractional digits ∉ {3,6}), cross-checks every value through `core.timeutil.parse_iso_utc`. **VERDICT: URGENT.** 38.27% of committed ISO timestamps (638,730 / 1,669,146 over the canonical `dt=` tape; **653,208 / 1,725,070 = 37.87%** incl. probe-cache `.jsonl`) would crash a strict 3.9 consumer. Dominant: bare `Z` (~99.6%); plus the literal L136 `.7Z`/`.71Z` in `sports_history.occurrence_datetime` (~36); plus a **NEW shape L136 never named — 9-digit NANOSECOND fractions** in `crypto_hourly.exchange_time` (exactly **1,409**). **0 genuinely-corrupt values**; `parse_iso_utc` parses **100%** including the nanosecond (truncates to 6) → the wrapper is the COMPLETE fix; the backlog is a live landmine, not latent. Finding: `findings/2026-07-24-tape-timestamp-parseability-audit.md`; lesson **L150** (closes L138's open question, extends L141 with tape-DATA evidence).

**Two-agent redundancy (not a verdict-class change, but applied anyway — descriptive census, same posture as L143).** A `tape-auditor` full-file scan and an adversarial `verifier` from-scratch re-derivation independently reproduced the broader-scope headline (1,725,070 / 653,208 / 37.87% / 0 unparseable / 0 wrapper failures) to the EXACT integer; a separately-built collector-engineer reproducer converged on the same 38.27% canonical-scope number. Four independent computations, identical verdict. NO registry change, NO P&L, NO bootstrap CI — a data-integrity characterization.

**Paper tier.** `execution/strategy_api.SHADOW_REGISTRY` = `{s14_ladder_underwriting}` (DEAD per Q34, paper-infra-validation only). `python scripts/paper_pass.py`: `paper: 0 open position(s), 907 settled contract(s), realized P&L $+15.69, cash $+15.69, open notional $0.00` — 0 newly processed (188 deferred caps / 264 deferred coverage / 112 already-in-ledger), idempotent, no new ledger lines.

**Gates.** `pytest -q` PASS / `python scripts/invariants.py --full` exit 0 (only pre-existing non-gating advisories: L25 dir-shape, L109 GC-dispatch, L74 daily-gap, L138/L141 raw-fromisoformat — the L141 one is exactly the 36-site backlog this audit proves is urgent). Duplicate-lesson-ID advisory clean (L150 next-free). Still **0 proven edges**.

## 2026-07-24 ~09-10 UTC — IDLE RUN (policy a): L142→L148 committed-tape JSON-validity gate (git-tracked scope) + L149 hourly_pass de-flake (research loop)

**Step 0/0a/0b.** Full independent Q0-Q47 rescan found the queue SATURATED (idle run): every item DONE / credential-or-auth-BLOCKED / calendar-gated-not-open / gate-open-but-density-inadequate. Reverified the two live gates directly against committed tape: Q19 FOMC gate opens 2026-07-29 (not open); Q36 Part-1 still 2/10 KXTEMPNYCH settled events in `tape/settlement_ledger/` (MIN_EVENTS=10 — still frozen; a new `dt=2026-07-22` settlement_ledger day added a 2nd KXTEMPNYCH event vs the 07-22 check's 1); Q43 `tape/perp_tape/dt=2026-07-24` only 2 distinct captures / 34 lines (density unchanged, still gated). Idle-run policy (a): the only other open UNENFORCED items are Ryan/policy-gated (L145 ws_depth `inv_order_endpoints_confined` collision, L22 `real_bid` enum) — neither is an in-authority unilateral conversion — so converted an UNENFORCED-lesson-class gap into a gating invariant instead.

**The milestone (L142 → L148).** Generalized L142's conflict-marker-ONLY committed-tape gate into a general JSON-per-line validity gate: `scripts/invariants.py` gained `_tape_invalid_jsonl_issues()` + `tape_invalid_jsonl_failure()`, wired into `main()`'s GATING tree-scan path alongside the existing conflict-marker gate, with explicit non-overlap so conflict-marker lines keep L142's own diagnostic and aren't double-reported. Scanned scope is git-TRACKED tape only (`git ls-files -- tape`, filtered to `.jsonl`), offline-safe (any git failure → empty set → check skips, never wedges the gate). ~10 new tests in `tests/test_invariants.py`; a HARD real-tree test pins the current committed tape clean (212 `.jsonl` files, ~1.06M non-empty lines, 0 malformed). New lesson **L148** (invariant/GATING + test).

**Two-agent trail (verifier CONFIRMED core, REFUTED two things — both fixed same commit).** A `verifier` independently re-scanned the real tape (→ 0 invalid, genuinely gating, no double-report, correct `*.jsonl` scope) and REFUTED: (1) the original raw-filesystem walk would false-fail on an UNTRACKED/in-flight collector mid-append (torn final line, no trailing newline) and wedge the autonomous loop on never-committed data → fixed by scoping to git-tracked files + regression tests pinning both directions (untracked torn line NOT flagged; tracked malformed line STILL flagged); (2) a pre-existing wall-clock flake — `tests/test_hourly_pass.py::test_main_wires_sports_limit_and_crypto_symbols` and `::test_main_returns_nonzero_on_incomplete_pass` never stubbed `hp.settlement_ledger`, so at UTC hour 10 (`SETTLEMENT_LEDGER_UTC_HOUR=10`) `hourly_pass.main()` made a REAL settlement_ledger network call that errored in-sandbox (the first test's assertion flaked, the second did live I/O silently) → fixed by stubbing settlement_ledger and the other hour-gated legs (weather_actuals/hour12, forecast_collector/hour11, universe_sweep/hours 0,6,12,18) so both tests are wall-clock-independent. New lesson **L149** (test) records the class.

**Stray-tape note.** A pytest run during this milestone triggered a live `hourly_pass` side-effect that wrote two real hour-9 captures (`tape/anomalies/dt=2026-07-24.jsonl`, `tape/econ_prints/dt=2026-07-24.jsonl`, `real_ask`, capture 09:14:23Z, off both cron minute-buckets) — real data, deliberately EXCLUDED from this commit (surgical path-adds), left untracked on disk so nothing real is destroyed. This live-test side-effect is itself an instance of L149's hour-gated-stub lesson.

**Gates.** `pytest` 1556 passed / 0 failed (baseline 1541); `python scripts/invariants.py --full` exit 0 (only pre-existing non-gating advisories: L25 dir-shape, L109 GC-dispatch, L74 daily-gap, L138/L141 raw-fromisoformat). No strategy claim, no registry change, no P&L/bootstrap CI — reliability infra, same class as L142/L141/L144; the verifier pass was warranted because this is a new GATING mechanism that could wedge the loop. Duplicate-lesson-ID advisory stays clean (L148/L149 use next-free IDs). Still **0 proven edges**.

---

## 2026-07-24 ~06:3x UTC — IDLE RUN (policy c-adjacent): lessons-ledger duplicate-ID collision found + fixed, verifier-caught citation gap corrected, + stranded-tape sweep (research loop)

**Step 0a/0/0b.** PASS. `main` fast-forwarded cleanly to HEAD `b8f229d` (PR chain #175->#184 all ancestors, no rewind); `kb/00-LOG.md`'s newest entry and newest committed tape both 2026-07-24. Claim-check: open PRs are #125 (leave-open-for-Ryan weekly retro) and #165/#166 (draft, Ryan-approved background-session infra) — none claim eligible queue work. Full independent Q0-Q47 rescan (first-Status-line-per-item, i.e. newest since entries are prepended): **0 eligible TODO/IN-PROGRESS** — every item is DONE / cred-or-auth-BLOCKED (Q33, Q42-pt3, Q47) / calendar-gated-not-open (Q19 FOMC 07-29, Q37 ~08-05) / gate-open-but-density-inadequate (Q36, Q43, both re-confirmed unchanged vs the 07-24 edge-hunter run) / RESERVED (Q17). IDLE RUN. Step 0b: swept the one eligible (>30min old) stranded branch, `tape/hourly-20260724T0406Z` — union-appended **2,213 missing lines** across 8 family files (`crypto_hourly` +2, `hyperliquid_funding` +2 new file, `orderbook_depth` +1297 new file, `perp_tape` +17 new file, `polymarket_macro_pairs` +15, `sports_pairs` +301, `weather_books` +533 new file, `weather_books/meta` +46 new file) into this run's commit; verified pure-append (no existing line touched/reordered) and 0 malformed JSON across all appended lines. The other 183 known stranded branches are pre-existing housekeeping debt (repeatedly flagged out-of-scope-for-one-run by prior audits); not touched.

**The finding.** Idle-run policies (a)/(b)/(d) had nothing fresh (UNENFORCED backlog empty since L144; every near time-gated item already self-activating; Q21 replenishment just ran at 04:1x). Took policy (c) on a genuinely new dimension: audited `kb/lessons/00-lessons.md` itself (the ledger every prior data-quality audit scans tape/code against, never itself) for internal integrity. Found **two duplicate lesson IDs**: `L130` was independently assigned to both a "Polymarket US public API — docs lie" lesson AND a "temporal-derivative depth feature / S46 mid-efficiency-wall" lesson; `L131` to both an "Authenticated != order-capable / ws_depth `inv_order_endpoints_confined` collision" lesson AND a "`two_sided_share` not maker-capturable / FILL WALL / S47" lesson — both collisions dated 2026-07-22 (two runs/agents picking a next ID without checking the ledger's current max), undetected through 2+ days of subsequent audits because they all scanned tape, not this ledger.

**The fix.** Citation-frequency check found the "temporal-derivative"/S46 and "FILL WALL"/S47 meanings are the actively-cited ones for L130/L131 (repeatedly referenced in Q21 rounds/idea-gen preps); renumbered the other two rows instead (narrative text byte-identical, ID column only): **L130 (Polymarket US probe) -> L146**, **L131 (ws_depth collision) -> L145**. Repointed `scripts/invariants.py::inv_order_endpoints_confined`'s 4 self-references (docstring, inline comment, error-message string) and one `tests/test_invariants.py` comment from L131 to L145. Added a non-gating advisory, `scripts/invariants.py::_duplicate_lesson_id_issues`/`duplicate_lesson_id_warning` (parses the ledger's `| L<n> |` ID column only, ignores prose mentions elsewhere in a row), wired into `main()`'s `--full` path — the real tree is now clean and stays that way going forward. New lesson **L147** documents the incident + the enforcement. 7 new tests in `tests/test_invariants.py`.

**Verifier round: REFUTED, then fixed (two-agent trail).** An independent `verifier` pass (this run) confirmed the mechanical de-dup was sound (regex correct, narratives untouched, gates green, no new advisory fires) but **REFUTED** L147's original citation-footprint claim — it said the two renumbered rows "were cited only inside this ledger and `scripts/invariants.py`," which is false: this append-only log itself cites the OLD meanings 3 times, at lines ~820/940/942 (`L131` = ws_depth collision, `L130` = Polymarket probe, both dated 2026-07-22). Because this file is append-only, those historical lines are NOT edited — L147's row text was corrected instead to acknowledge the gap honestly, and this entry serves as the explicit reconciliation a reader needs:

**Reconciliation table — read this if an entry above (dated on/around 2026-07-22, roughly lines 800-950) cites `L130` or `L131`:**
| citation you're reading | old meaning (what that historical entry meant) | current ledger ID for that meaning |
|---|---|---|
| `L131` (e.g. "the collision L131 documented and predicted") | ws_depth / `inv_order_endpoints_confined` auth-header invariant collision | **L145** |
| `L130` (e.g. "vendor 'public endpoint' docs are not load-bearing") | Polymarket US public API probe ("docs lie, probe first") | **L146** |
| `L131` (e.g. "authenticated != order-capable") | ws_depth collision (same as above) | **L145** |
| any OTHER `L130`/`L131` citation (Q21 rounds, idea-gen preps, most of `LOOP-QUEUE.md`/`findings/`) | temporal-derivative/S46 (L130) or FILL WALL/two_sided_share/S47 (L131) | **unchanged — still L130/L131** |

New lesson candidate (verifier's, folded into L147 rather than a separate row): renumbering a colliding lesson ID must grep the OLD ID across the whole repo INCLUDING the append-only log before claiming a citation footprint, and — because the log can't be edited — must pair the renumber with an appended reconciliation entry exactly like this one. The `_duplicate_lesson_id_issues` advisory only catches intra-ledger duplication; a stale cross-file citation to a since-renumbered ID is a distinct failure mode it cannot detect, and stays a known gap (not statically assertable without parsing every historical log entry's prose for lesson-ID mentions — not attempted here).

**Gates.** `pytest`: 1534 -> **1541 passed** (+7 new duplicate-lesson-ID tests). `python scripts/invariants.py --full`: exit 0, same 4 pre-existing non-gating advisory classes, no 5th (the new advisory is clean on the real tree). Two-agent rule: N/A for a registry/CI verdict (none made) but voluntarily applied anyway given this touches a gating invariant's (`inv_order_endpoints_confined`) docstring/messages — verifier round completed and its finding fixed before commit, per the spirit of the rule. No strategy claim, no registry change, no P&L claim, no execution/credential change.

**Step 9 (paper sub-pass).** `SHADOW_REGISTRY={s14_ladder_underwriting}` (DEAD-at-real-fills per Q34 — paper-infra validation only, NOT edge evidence). `scripts/paper_pass.py` run against the freshly-swept tape: 0 newly-eligible events for S14 this pass (188 deferred-caps, 264 deferred-coverage, 112 already-in-ledger) — ledger unchanged, realized P&L **+$15.69** (`broker_truth`). Still **0 proven edges**.

---

## 2026-07-24 ~04:1x UTC — kalshi-edge-hunter (nightly): Q21 idea-gen round (S48/S49/S50 all KILLED, 0 registered) + adversarial review CLEAN + probe-prep no-op

**Nightly thinking-seat run, 3 units.** Step 0a/0/0b clean (main HEAD `9d6506a`, PR chain #175→#183 unbroken, kb-log + tape current 07-24, no rewind; claim-check found only leave-open/draft PRs; 183 `tape/hourly-*` + 1 `tape/burst-*` remote branches — long-tail sweep debt, out of scope for one run).

**Unit 1 — ADVERSARIAL REVIEW (clean).** The last 24h produced 4 findings, all data-quality/idea-gen prep (no strategy verdicts, so no bootstrap-CI/fee-rate/provenance verdict to overturn). Re-checked the one load-bearing number — the `2026-07-23-universe-sweep-family-shapes.md` schema-defect claim — independently over the full committed `tape/universe_sweep/`: `no_ask_size`/`no_bid_size` are 0.0 on **100% of all 460,000 lines** (collector maps only `yes_*_size_fp`, `collection/universe_sweep.py:116-117`), the NO/YES mirror `no_ask==1-yes_bid` holds **460,000/460,000 (100.00%)**, and `yes_ask_size` nonzero **3.4059%** — every number reproduces. Nothing failed the re-check → no GitHub issue opened. (The defect itself stays flagged for the Ryan-gated Q46 collector lane.)

**Unit 2 — Q21 REPLENISHMENT (0 registered, 11th consecutive zero round).** 0 eligible items → round fired. Producer proposed 3 falsifiable candidates; an independent `verifier` attacked each against committed tape and KILLED all 3: **S48** commodity daily-close settlement-basis ρ-guard (KXWTIH/KXGOLDH/KXSILVERH) — UNVERIFIABLE (no committed commodity spot/settlement reference in `tape/`; settlement join is exactly n=1, `KXSILVERH-26JUL2205` partial; quoted-but-untraded census, `volume_24h>0` on 2/440 KXWTIH rows) → L9/L43 + L96 + S1/weather overround, needs a dedicated collector; **S49** perp-funding-sign directional predictor for the crypto hourly binary — calibration hit-rate 49/100=0.490 (below a coin flip), `funding_rate_estimate=0.0` on 95/95 BTC + 95/95 ETH (S42/L105 clamp), and `crypto_hourly` is a 188-member "between" range-pin ladder (bracket_sum 3.53) not a directional binary (L88); **S50** weekend stale-ask maker fade on commodity ladders — FILL WALL (L131, no trade-print field) + zero commodity rows on `dt=2026-07-18/19` + OI≈0 + flat-1¢-maker-fee eats the 1–3¢ spread (S6/S13). Diversity floor met (S48 = new market family). Consumed S48/S49/S50 → next free **S51**. NO registry table change (prose-note precedent). Lesson candidate deferred to kb-distiller: *commodity ladders in `tape/universe_sweep/` are a quoted-but-untraded census (OI≈0, no spot ref, settlement join n=1) inheriting L96*. See `findings/2026-07-24-q21-idea-gen-round.md`.

**Unit 3 — PROBE-PREP (no-op).** Nothing unblocks within ~72h that isn't already prepped: FOMC (Q19) is 07-29 (5 days, probe prepped), Q37 ~08-05, Q36/Q43 data/Ryan-gated.

**Housekeeping.** No open PR needs flagging (#125 is the by-design leave-open weekly-retro, repeatedly logged as NOT-re-flagged; #165/#166 are 1-day-old Ryan-approved drafts). Two passed-event burst triggers still `enabled=true` and re-firing annually named for deletion: `kalshi-burst-wcfinal-0719` (event Jul 19), `kalshi-burst-wcsemi2-0715` (event Jul 15); `fomc-0729` kept (fires Jul 29). Remote branch count: **183** `tape/hourly-*`, **1** `tape/burst-*`.

**Gates/scope.** `pytest` 1534 passed; `python scripts/invariants.py --full` exit 0 (same 4 pre-existing non-gating advisories: directory-shaped dt, GC-orphan classes, 12 daily-cadence gaps, 36 raw-fromisoformat sites). Docs/findings-only diff; two-agent rule satisfied (producer + verifier, all KILL). Step 9 paper sub-pass: SHADOW_REGISTRY={s14_ladder_underwriting} (DEAD per Q34 — paper-infra validation only, NOT edge evidence), no new tape committed this run to replay, ledger unchanged **+$15.69** (`broker_truth`). Still **0 proven edges**.

---

## 2026-07-24 ~0x:xx UTC — IDLE RUN (policy a): meta-guard for unregistered single-hour committed collector legs — closes L123's structural residue, +settlement_ledger to DAILY_CADENCE_FAMILIES (research loop)

**4th idle run in a row.** Full independent Q0-Q47 rescan (latest Status line per section, file-shape verified per L25): **0 eligible** TODO/IN-PROGRESS — all DONE / cred-or-auth-BLOCKED (Q32 both-legs, Q33, Q42-pt3, Q47-activation) / calendar-gated-not-open (Q37 ~08-05) / gate-open-but-density-inadequate (Q36, Q43). Idle-run **policy (a)** taken — and, contra this session's own ~00:2x note that called policy (a) "exhausted," it was NOT: that note reasoned from L117's monitor-attribution candidate (indeed already built), but the STRUCTURAL residue of **L123** was still genuinely open and, more concretely, `settlement_ledger` was **still missing from `scripts/invariants.py::DAILY_CADENCE_FAMILIES`** — the exact "registered nowhere so its freeze is invisible" gap that bit `weather_actuals` (L126) and `settlement_ledger` itself (L123/L124/Q36).

**What I built.** (1) Added `"settlement_ledger"` to `DAILY_CADENCE_FAMILIES` (`scripts/invariants.py`), tying it to `SETTLEMENT_LEDGER_UTC_HOUR=10` and the L123/Q36 freeze; grepped `hourly_pass.py`/`settlement_ledger.py` first for any DELIBERATE cadence-monitoring exclusion — found none (matches the finding). (2) Added a deterministic, offline, NON-gating **meta-guard** (`_unregistered_single_hour_leg_issues`/`unregistered_single_hour_leg_warning`, wired into `main()`'s `--full` advisory path like L74/L109/L138/L141): it parses `collection/hourly_pass.py`, finds every single-hour committed leg (`ts.hour == *_UTC_HOUR`, EXCLUDING the plural `*_UTC_HOURS` `universe_sweep` set-gate), resolves the committed tape family/families each writes via `SINGLE_HOUR_LEG_FAMILIES`, and advises if any is not in `DAILY_CADENCE_FAMILIES` and not documented-exempt (`FORECAST_COLLECTOR_UTC_HOUR -> gitignored data/forecast_tape/`). An UNRECOGNIZED new `*_UTC_HOUR` is SURFACED, not silently passed — so the NEXT single-hour committed leg added without registration trips CI instead of freezing invisibly (the loop L126 and this run had to close by hand). Best-effort/exception-swallowing so it can never poison the gate.

**Lesson superseded: L123** (its enforcement cell carried the exact "candidate (a): a `scripts/invariants.py` advisory … that asserts every once-per-day exact-hour leg …" fingerprint). New row **L144** moves its STRUCTURAL registration-completeness half UNENFORCED -> **invariant/test**; honest scope recorded: the trailing-edge WALL-CLOCK freeze half is already L124's `tape_gap_monitor.py` runtime STALE check (deliberately NOT duplicated in the offline gate — that would be flaky/non-deterministic), and `_daily_family_gap_issues` still skips a <2-dt-file family (interior-gap only).

**One VERIFIED deviation from the finding (flagged, not noise).** The finding assumed `settlement_ledger` had exactly ONE committed dt file (so registering it would add no interior-gap advisory). It now has TWO (`dt=2026-07-17` build day + a fresh `dt=2026-07-22` VPS pass, committed at `2c30781`), so the existing **L74 daily-cadence advisory correctly grew 8 -> 12 missing days**, surfacing settlement_ledger's genuine `07-18..07-21` freeze — the very Q36 blocker the finding cites. This is a TRUE POSITIVE inside the pre-existing 4th advisory CLASS, **not a new 5th advisory** and **not the meta-guard firing** (the meta-guard is clean on the real tree). The advisory SET is unchanged (still 4 classes: directory-shaped tape days / GC-orphan classes / daily-cadence gaps / raw-fromisoformat sites); only the day-count inside the daily-cadence advisory changed.

**Gates.** `pytest` **1524 -> 1534 passed** (+10 tests, all in `tests/test_invariants.py`). `python scripts/invariants.py --full` **exit 0**, same 4 pre-existing non-gating advisory classes; the new meta-guard adds **no** advisory firing on the real tree (passes once `settlement_ledger` is registered, exactly as designed). NO collector/network change, NO strategy claim, NO registry touch (`kb/strategies/00-index.md` untouched), NO CI, NO P&L, NO demo/live path, no execution/ or credential change. Two-agent verdict rule N/A (non-gating invariant/test addition — L118/L126/L104/L110/L141 precedent).

**Files.** `scripts/invariants.py` (DAILY_CADENCE_FAMILIES += settlement_ledger; SINGLE_HOUR_LEG_FAMILIES/SINGLE_HOUR_LEG_EXEMPT maps + meta-guard + main() wiring); `tests/test_invariants.py` (+10 tests); `kb/lessons/00-lessons.md` (L144); this log; one LOOP-QUEUE run-log line.

---

## 2026-07-24 ~00:2x UTC — IDLE RUN (policy c): perp_tape multiplexed-record_type consumer-correctness audit — CLEAN, + regression-test guard on the gate-open probe (research loop)

Full independent Q0-Q47 rescan (latest Status line per section): 0 eligible TODO/IN-PROGRESS — all DONE / cred-or-auth-BLOCKED (Q1-odds, Q33, Q42-pt3, Q47) / calendar-gated-not-open (Q19 FOMC 07-29, Q37 ~08-05) / gate-open-but-density-inadequate (Q36, Q43). Q43's calendar gate is open (perp_tape 7 file-shaped days) but density is unchanged from the edge-hunter's 07-23 run (perp_tape 07-23=51 lines, crypto_hourly 07-23=10; no tape past 07-23 22:01 in this clone), so re-running its probe would only reproduce yesterday's data-inadequate result — not a new milestone. Idle-run policy (a) is exhausted (L117's proposed `tape_gap_monitor.py` minute-of-hour collector-attribution enhancement is ALREADY BUILT — the `collectors`/`collector_diagnosis`/`EXPECTED_COLLECTOR_BUCKETS` machinery exists; its ledger row is a stale UNENFORCED marker; the other open UNENFORCED rows are Ryan-gated or per-probe methodology, not statically assertable). Policy (b): every near time-gated item already carries a prepped self-activating probe. Policies (c)/(d) both fired multiple times 07-23 (L142/L143/universe_sweep breadth), so this run took **policy (c) on a genuinely NEW dimension**.

**The audit (NEW dimension).** Cross-record duplicate-capture integrity + multiplexed-record_type consumer-correctness, over the actively-collected tape families. Findings, all CLEAN: (1) keyed by `(record_type, ticker, captured_at)` there are ZERO byte-differing duplicate rows in any active family — the apparent same-`(ticker,captured_at)` collisions in `perp_tape` are just distinct record types written in one pass (an `orderbook` BBO row and a `funding_estimate` row legitimately share a capture timestamp), not real duplication; the stranded-tape sorted-line-set union-merges (Q47) already remove exact dups. (2) `tape/perp_tape/` is the SOLE multiplexed-record_type family — 4 types (`markets`/`orderbook`/`funding_estimate`/`funding_rates`) sharing `(ticker, captured_at)` per pass; every other active family is single-type. (3) Consumer-correctness: all three current perp consumers filter `record_type` correctly — `q43_perp_binary_consistency_probe.load_perp_bbo` (`record_type != "markets": continue`, and `orderbook`/`funding_estimate` records structurally lack a `contracts` list so the filter is belt-and-suspenders), `q42_funding_clamp_probe` (`== "funding_rates" and mode`), `q42_crossvenue_funding_join` (`!= "funding_rates"`/`!= "funding_history"`). So there is NO L96-class record-type conflation anywhere in the gate-open item's read path — an important negative to have on record before Q43's densification run writes new perp consumers.

**Compounding artifact.** Since nothing pinned `load_perp_bbo`'s filter against a future refactor (its 18 tests only exercised the `markets`-record happy path), added one adversarial regression test `test_load_perp_bbo_ignores_non_markets_records` (tests/test_q43_perp_binary_consistency_probe.py): an `orderbook`/`funding_estimate` record carrying a top-level perp ticker AND a bogus `contracts` list with a DIFFERENT quote for the same `(ticker, captured_at)` must NEVER produce a BBO — only the `markets` record's contracts do. This protects the ONLY gate-open item's correctness. Lesson-candidate (kept in this log, not appended to kb/lessons/ to avoid a backlog merge conflict, per the 07-23 posture): *perp_tape is the sole multiplexed-record_type tape family; any BBO consumer must filter `record_type == "markets"` and any funding consumer `funding_rates`/`funding_estimate` — verified all 3 current consumers do.*

**Scope/tags.** tests/test_q43_perp_binary_consistency_probe.py (+1 test), one appended Q43 status line, this log entry, one run-log line. NO collector change, NO strategy claim, NO CI/P&L, NO registry touch — two-agent verdict rule N/A (data-quality audit + regression test, same posture as L142/L143/Q44-46). `pytest` 1523->**1524 passed**. `python scripts/invariants.py --full` **exit 0** (same 4 pre-existing non-gating advisories: directory-shaped tape days, GC-orphan classes, 8 daily-cadence gaps, 36 raw-fromisoformat sites; no new advisory from this diff). Step 9 paper sub-pass: SHADOW_REGISTRY={s14_ladder_underwriting} (DEAD per Q34, paper-infra validation only, NOT edge); `paper_pass.py` processed 10 newly-eligible events, realized P&L +$15.15 -> **+$15.69** (`broker_truth`; 907 settled contracts, 0 open). Still **0 proven edges**.

---

## 2026-07-23 ~21:2x UTC — IDLE RUN (policy c): cross-family silent-zero field census — no new defect (research loop)

Full independent Q0–Q47 rescan: 0 eligible TODO/IN-PROGRESS (all DONE / cred-or-auth-BLOCKED [Q1-odds, Q33, Q42-pt3, Q47] / calendar-gated-not-open [Q37 ~08-05] / gate-open-but-data-inadequate [Q36 both legs, Q43 density — Q43 already run today by the edge-hunter]). Matches today's earlier runs. Idle policies (a)/(c)/(d) already fired today (L140/L141/L142/Q21-prep).

Policy (b) evaluated and honestly had nothing clean to grab: every NEAR time-gated item already carries a prepped self-activating probe (Q43 q43_perp_binary_consistency_probe, Q37 q37_weather_summer_makerno_probe, Q36-p1 q36_kxtempnych_settlement_basis_probe, Q19/Q12 s17_leadlag_probe --burst-window). The only un-prepped "time-gated" items are data-surface/signal-blocked, not genuinely time-gated: Q36-p2 microstructure needs a Ryan-side intra-hour collector (hourly cadence caps KXTEMPNYCH at ~1 book snapshot/market-hour); Q10/S12's nowcast-overlay signal is `not_built` for CPI/payrolls and exists only for quarterly GDPNow (confirmed live), so its ≥20-release gate is unreachable. Prepping either is make-work → fell back to policy (c).

Ran a cross-family silent-zero/null/constant numeric-field census + source-tag-enum integrity check over all 13 actively-collected tape families (newest committed day each, 4000-line cap on big families) — a NEW data-quality dimension, distinct from L142's conflict-marker scan (PR #177). Verdict: NO new L96-class collector-mapping defect beyond universe_sweep's already-documented no_ask_size bug (L96/Q21) — the L96 defect is bounded, not systemic. Every other always-zero/constant/null flag resolves to legit data (crypto yes_bid=0 wings L26/L31; perp funding_rate~0 the Q42-confirmed ±1bp clamp), a config constant (tick_size=0.0001, n_sources=2, tolerance_F=1.0, n_markets=6), an honest-accounting nullable (ticker_parse_error, is_provisional, expiration_values_disagree), or a broker_truth-filter consequence (weather_actuals.*spread=0). Source-tag integrity clean ({real_ask,broker_truth,synthetic} only, 0 MISSING; orderbook_depth uses the plural `price_source_tags` map; anomalies/analysis records legitimately carry none). Confirmed settlement_ledger.expiration_value is correctly nullable-by-market-type (populated 72.0/73.0 for the 2 settled KXTEMPNYCH events, null for binary combo/sports — a census line-cap artifact, not a defect). No registry change, no P&L, no bootstrap; two-agent rule N/A (data-quality audit, L142/Q44–46 precedent). New lesson L143 (ledger-only). Still 0 proven edges.

OPS note for Ryan (issue #157): the research-loop sandbox's python env was stripped — pytest was installed under an interpreter lacking `requests`, and the system `cryptography` panicked with `pyo3_runtime.PanicException` root-caused by `ModuleNotFoundError: No module named '_cffi_backend'`. Installing `cffi` (plus pytest/websocket-client/openpyxl/numpy into the requests-having /usr/local/bin/python 3.11.15) resolved the panic and the full suite went green — issue #157's "cryptography/cffi pytest panic" is fixable in-env by `pip install cffi`. After env repair: pytest 1523 passed (exit 0), invariants --full exit 0.
