# Dead-candidate notes + update chronology — verbatim archive

`archive` · split out of `00-index.md` on 2026-07-15 (Ryan session, context-bloat trim:
the index is read in full by every research run; dead-candidate narrative and the dated
update chronology are consulted on demand, not per-run). Nothing here was edited in the
move — blocks are verbatim. The registry of record stays `00-index.md` (its table rows,
including full verdict text for dead candidates, are untouched). Consult this file when
arguing a new candidate "survives its nearest dead cousin" (Q21 rule).

## Dead-candidate note blocks (moved from "Notes on each" + candidate sections)
...
## The one rule — dated update chronology (moved verbatim; newest at bottom)
