...
| **S28** | Post-close settlement-lag taker on decided sports outcomes | 2026-07-15 Q21 gen · settlement/close-time mechanics (diversity floor) × Q25 post_close cells | **idea** | low | queue-aware post_close taker lift of a sub-fair winner-side `real_ask` on a DECIDED game (ttc past a margin exceeding the Q25 sports-tz uncertainty plus game duration, coarse/date-only tickers excluded), net taker 0.07 (`core.pricing`); block-boot by GAME, `bootstrap_verdict_admissible` + `clears_tick_magnitude`. Kill: post_close asks converged ≥~$0.98 / lookahead-clean pop <10 games / L26 mirror non-price / CI fails gate. See Q29. |
