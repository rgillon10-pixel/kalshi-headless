...
| **S68** | Two-sided both-bid overround-capture maker on wide-spread genuine game moneylines | 2026-08-01 Q21 gen · maker BID-side mirror of Q6's ask-side arb sweep × the wide-spread two-sided game-book population no prior maker probe isolated | **dead ✗** | low | REGISTERED at idea stage (2026-08-01, kalshi-edge-hunter Q21 round #19, producer + independent `verifier`, verifier returned SURVIVE on this candidate / KILL on S66,S67; producer independently re-derived every load-bearing number to the digit). **NOT an edge — a deterministic GROSS overround with a defined binding test, no CI.** Mechanism: on 2-outcome game moneyline books whose two-sided spread ≥ 2× the flat maker fee, rest BOTH a YES bid and a NO bid; if both fill you own both sides for `yes_bid+no_bid` < $1 and one settles $1. Data (both already collected): `tape/settlement_ledger/` (`broker_truth` result+close_time) ex-post joined (L50) to `tape/orderbook_depth/` (`best_yes_bid`/`best_no_bid`, `real_bid`). Idea-stage evidence: window overlaps (settle 07-07→07-22 ⊂ depth 07-07→08-01, L9 N/A); **205** settled tickers with a pre-close genuinely-two-sided depth snapshot and yes-spread ≥ 2¢, across **16 real game series** (KXUECLGAME 53/KXNPBGAME 47/KXKBOGAME 19/… — clears L41 ≥10 floor, clears L31, NOT the KXMVE nominal-wing artifact); mean yes-spread **7.31¢**, `yes_bid+no_bid` mean **0.927** and < 1 for **100%** of the 205 → both-bid capture **+7.31¢ gross / +5.31¢ net** of the 2¢ two-sided maker fee (`core.pricing`); taker leg on the same books −6.87¢ (S52/S1 overround wall). **Load-bearing caveat scoping the probe: the +5.31¢ is a BOTH-SIDES-FILL number.** Single-side optimistic cuts do NOT robustly clear zero — maker-YES@bid CI [−2.42¢,+3.89¢] straddles zero; maker-NO@bid +4.24¢ is a NO-base-rate artifact (62% settle NO). So the entire edge is the unmodeled maker fill-rate/adverse-selection question — the L5 wall that killed S6/S13/S14/S23. Binding test (**Q49**): queue-aware BOTH-sides-fill maker sim over `orderbook_depth` `yes_bids`/`no_bids` ladders (S6/S19/S21/S23 L2-queue methodology, L39-free) with an explicit adverse-selection model, block-boot by GAME-SERIES (L6/L41), net of fee via `core.pricing`; CI>0. Kill: fill rate below the S19 floor / realized-fill overround ≤0 once adverse selection is charged / CI≤0. Presumptive outcome: KILL on adverse selection. Prices `real_bid` (queue) + `broker_truth` (settlement). See `findings/2026-08-01-q21-idea-gen-round.md`. **(Q49 PROBE RUN 2026-08-01, research loop — VERDICT **DEAD (edge)** but recorded **PROVISIONAL**; **STATUS DELIBERATELY UNCHANGED at `idea`**, no registry flip, because the two-agent rule is UNSATISFIED: that run's harness exposed no subagent-dispatch tool so no independent `verifier` could be dispatched. S55 precedent for an in-place clause with no status word change. `scripts/q49_two_sided_maker_fillsim.py` + 44 offline tests; finding `findings/2026-08-01-q49-s68-two-sided-maker-fillsim-verdict.md`. **Structural result: if BOTH legs fill the P&L is arithmetically deterministic and adverse selection cannot touch it** — one YES + one NO on the same binary book, exactly one settles $1 regardless of outcome — so the both-fill-conditioned cut reproduces the +5.31¢ BY CONSTRUCTION and proves nothing (L5); the deployable unit is the PER-ATTEMPT P&L over both/YES-only/NO-only/neither. Binding cell (entry `late` = the idea-stage regime, min-spread 2¢, fill model `queue_price_through`): **564 attempts / 18 game series / 316 games**, both-fill rate **21.81%** (≈48× the S19 0.45% floor ⇒ dies on the EDGE not adequacy, L53), both-fill cut **+$0.0115** net of the 2× flat 1¢ maker fee (`core.pricing`), but **PER-ATTEMPT block-boot by GAME-SERIES mean −$0.0923, 95% CI [−0.1311, −0.0423]** fully below zero, `bootstrap_verdict_admissible` PASS (18 units) / `clears_tick_magnitude` FAIL, 17/18 series negative (123 both-fills at +$0.0115 vs **247 single-leg fills at ≈−$0.21**). **Adverse selection MEASURED not assumed:** P(settles YES \| YES-leg-only fill, n=126) **0.127** vs base 0.418 (0.30×); P(settles NO \| NO-leg-only fill, n=121) **0.248** vs base 0.582 (0.43×). **Entry-policy confound stated:** `late` truncates the window to a median 3 post-entry snapshots (vs 65 under `first`), so the two cells are CO-BINDING and the verdict rests on the weakest sufficient claim — **none of the 4 cells (2 entry policies × 2 fill models) yields an admissible CI > 0 clearing the tick gate**; the fairer full-life cell (`first` + `queue_price_through`) is a wash at **+$0.0011, CI [−0.0533, +0.0797]**. **Non-circularity:** the price-movement-BLIND `queue_only` model also fails (+$0.0083, CI [−0.0014,+0.0277], tick-gate FAIL) and its `first`-entry +$0.1617 is an L41-degenerate `no_opposing_unit` bootstrap (18/18 same-sign, 98.0% both-fill) — four cells, four failures, three routes; wide-spread sensitivity does not rescue it (5¢ −$0.0533 CI [−0.0776,−0.0293]; 7.31¢ −$0.0357 CI [−0.0817,+0.0043]). **NUMERIC CORRECTION to this row's own idea-stage figures:** the **205 / 16 series / 7.31¢ / 0.927** triple did NOT reproduce exactly under any snapshot-selection rule tried (nearest: 179/15 at 7.11¢/0.9289; 232/16 at 6.69¢/0.9331) and must be treated as approximate provenance, not a citable statistic — the qualitative claims DO reproduce (`yes_bid+no_bid` < $1 for **100.0%**, 15–18 game series ≥ the L41 floor). Same short-the-spread maker factor family as S6/S13/S19/S23 (Hard-Rule-#6 ρ cap, not diversification). **Pending action: a `verifier` re-run of `--entry-policy late`; on CONFIRM flip `idea` → `dead ✗`.**)** **(VERIFIER CONFIRMATION, 2026-08-01, independent `verifier` subagent — two-agent rule now SATISFIED, STATUS FLIPS `idea` → `dead ✗`.** Re-ran all 4 cells live off committed tape, reproduced every headline number to the digit (counts, fill rates, both-fill net, per-attempt means, adverse-selection rates). Attacked and could not break: settlement-join correctness (binary-filtered via `core.settlement.is_binary_result`, no synthetic leakage), fee routing (`core.pricing.fee_per_contract` at `MAKER_FEE_RATE`, flat 1¢/leg, not hand-rolled), lookahead (fill sim structurally outcome-blind, pinned by a dedicated test), a temporal-consistency tightening of model B (identical result), and — the entry-policy confound this row's own text raised — window-truncation controls (+$0.015–0.020 at matched window lengths, nowhere near −$0.0923): **the `late`/`first` sign flip is a genuine entry-regime effect (4.09¢ vs 18.49¢ mean spread), not a window artifact.** A by-GAME secondary bootstrap tightens, not loosens, the binding cell (CI [−0.1200,−0.0650]). A lookahead-free clock-based-entry steelman (H∈{2,6,12,24,48}h) is negative under the price-aware model at every horizon; its one passing cell (≤48h, price-BLIND model, +$0.0868) is a 96.2%-both-fill turnover-proxy artifact of exactly the kind L48 exists to exclude, not a live cell. **Three write-up corrections, none reversing the verdict:** (1) unanimity was misreported as 17/18 series negative — actual is **16 strictly negative, 1 exactly $0.0000 (`KXUSLGAME`, n=2), 1 positive (`KXWCGAME`, n=2)**; (2) median NO-side `queue_ahead` was 705.5, not 707; (3) the adverse-selection table (0.127/0.248 vs base 0.418/0.582) is **model-B-conditioned and partly circular** — the price-blind control shows a materially weaker effect (0.263/0.483, both n<40, z<2) and must be labelled as such, not as an independent confirmation of the mechanism. New lesson candidates (UNENFORCED, standing idle-run work): a fill-condition-conditioned outcome statistic is definitional not empirical unless read from a price-blind control; a unanimity count must separate strictly-negative from exactly-zero units. See the corrected `findings/2026-08-01-q49-s68-two-sided-maker-fillsim-verdict.md`.)** |
