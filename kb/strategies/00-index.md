...
| **S19** | Elevated-wing stale-ask maker fade on crypto ladders (S10-maker / L26 direction) | 2026-07-13 Q21 gen · S10-maker-untested × L26 | **idea** | low | maker short-YES rested on stale far-OTM `wing_elevated` members (yes_ask 0.20–0.67, yes_bid=0, >±3 strikes from spot per Q20); binding gate = queue-aware `orderbook_depth` `no_bids` fill-sim (NOT candlestick proxy — L39), P&L conditioned on the fill↔settlement adverse-selection correlation, block-boot by event-hour, CI>0 clearing the L27 tick gate; kill on 0%-fill null OR CI≤0 — a cheap L26/S10-maker-side closer, expected DEAD |
