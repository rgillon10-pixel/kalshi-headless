...
Status: DONE (2026-07-08) — all 4 hosts REACHABLE this run (Kalshi REST end-to-end verified via
`capture_orderbooks --limit 3`, Coinbase/Kraken 200s, the-odds-api 401-reachable); org egress
policy widened since 2026-07-02. See `tape/cloud-env-check.md`. `ODDS_API_KEY` still absent
(Q1 odds leg stays `BLOCKED(key)`).
...
Status: DONE (2026-07-08) — `collection/sports_pairs.py` built (16 new tests: ticker grammar,
2/3-way moneyline group filter, de-vig math, offline capture w/ honest partial-completeness);
first live pass captured 429 moneyline groups / 976 legs (real_ask) across 37 series incl.
4 World Cup games → `tape/sports_pairs/dt=2026-07-08/`. `ODDS_API_KEY` absent → odds leg
`BLOCKED(key)` per spec (Kalshi leg captured regardless, `odds_status="no_key"` on every line).
Not yet wired to run automatically — that's Q3 (hourly entry point).
...
Status: TODO (unblocked 2026-07-08 — see Q0b)
...
Status: BLOCKED(needs Q1 + Q2 built — egress itself unblocked 2026-07-08, see Q0b)
...
Status: TODO (unblocked 2026-07-08 — see Q0b; S7a still needs a historical odds source, free TBD)
One stage per run:
...
- 2026-07-08T15:09Z · Q0b · all 4 hosts now REACHABLE (egress policy widened since 2026-07-02); Q0b DONE, Q1/Q2/Q4/Q5/Q6 flipped BLOCKED(egress)→TODO, Q3 still BLOCKED(needs Q1+Q2 built). ODDS_API_KEY still absent. Proceeded to Q1.
- 2026-07-08T15:16Z · Q1 · built collection/sports_pairs.py + 16 tests; first live pass: 429 moneyline groups / 976 legs (real_ask), 37 series incl. 4 World Cup games, 0 incomplete; odds leg BLOCKED(key). Q1 DONE.
