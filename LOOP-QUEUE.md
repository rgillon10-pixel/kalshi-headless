...
Status: DONE (2026-07-09) — all 4 hosts now reachable (Kalshi 200, Coinbase 200, Kraken 200,
the-odds-api 401 MISSING_KEY = reachable, no key). Egress allowlist was widened since Q0's
2026-07-02 check. Every `BLOCKED(egress policy)` item below flipped back to TODO; see
`tape/cloud-env-check.md` for the refreshed evidence.
...
Status: DONE (2026-07-09) — `collection/sports_pairs.py` built + 19 unit tests (ticker parsing,
de-vig math, completeness, mutual-exclusivity filtering) + first live pass: 500 events across
42 open series (World Cup first), all `real_ask`, all complete. Odds leg `BLOCKED(key)`
(`ODDS_API_KEY` absent — Kalshi leg captured regardless per spec). See `kb/00-LOG.md`.
...
Status: TODO (unblocked 2026-07-09 — see Q0b; egress confirmed for Kalshi + Coinbase + Kraken)
...
Status: BLOCKED(needs Q2 — Q1 is DONE; egress itself is no longer the blocker, see Q0b)
...
Status: TODO (unblocked 2026-07-09 — see Q0b; S7a still needs a historical closing-odds source, which is a data-availability question, not egress)
One stage per run:
...
Status: TODO (unblocked 2026-07-09 — see Q0b)
...
- 2026-07-09T05:16Z · Q0b+Q1 · egress UNBLOCKED (all 4 hosts reachable; odds-api still needs a key) → flipped Q1–Q2,Q4–Q6 back to TODO; built `collection/sports_pairs.py` (19 tests) and ran a live pass: 500 events / 42 open moneyline series (World Cup first), all `real_ask`, all complete; odds leg `BLOCKED(key)`.
