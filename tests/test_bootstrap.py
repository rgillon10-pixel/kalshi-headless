...
#: The FROZEN 2026-08-09 snapshot of Q54/S79's first (and verdict-bearing) firing. The LIVE
#: `reports/q54_s79_flow_continuation.json` is REGENERATED by
#: `scripts/q54_s79_flow_continuation_probe.py`, which globs every committed trade day — so it
#: grows whenever new `tape/kalshi_trades/` days land (24 units/133 obs on 2026-08-09 ->
#: 45 units/214 obs on 2026-08-12). Pinning exact counts against the live path made this test a
#: time bomb that any re-run of the probe detonates; that is L325's rule (a mutable artifact
#: pinned by a test must be frozen for its pinning reader) recurring in `reports/` (L341).
Q54_FROZEN_REPORT = "q54_s79_flow_continuation-2026-08-09.json"
#: Identity pin on the frozen snapshot, so the comparand cannot be silently swapped (L284/L325).
Q54_FROZEN_REPORT_SHA256 = "8c58c7a59e1f57dd28dadf5815baa359bd000c3cfd20b012b97af9651df3d951"


def _frozen_q54_report_path():
    from pathlib import Path
    return Path(__file__).resolve().parent.parent / "reports" / Q54_FROZEN_REPORT


def test_acceptance_q54_s79_frozen_report_reproduces_l322s_effective_n():
    """Real-tape acceptance test (L322), read off the FROZEN 2026-08-09 snapshot (L325/L341).

    Q54/S79's verdict-bearing bootstrap pooled 133 entries into 24 game-units — the exact
    ragged-block population that motivated L322. Reproducing it from that run's own
    `population.entries_per_unit` sizes pins the ~11.6 Kish n (1.16x the L41 floor of 10, not
    the 2.4x the raw `n_units` implies) against real committed numbers rather than a synthetic
    fixture alone. It reads the frozen snapshot, never the live report, because the live one is
    a mutable artifact the probe rewrites every time it fires."""
    import hashlib
    import json

    p = _frozen_q54_report_path()
    assert p.is_file(), f"frozen snapshot missing: reports/{Q54_FROZEN_REPORT}"
    raw = p.read_bytes()
    assert hashlib.sha256(raw).hexdigest() == Q54_FROZEN_REPORT_SHA256
...
def test_acceptance_l322_pin_does_not_read_the_mutable_live_report():
    """L341: the pinning reader must not name the live, probe-regenerated artifact at all.

    `scripts/q54_s79_flow_continuation_probe.py` self-activates over every committed trade day,
    so `reports/q54_s79_flow_continuation.json` legitimately changes without anyone editing a
    test. A pin against it is red-on-correct-data waiting to happen (the L325 pattern, second
    occurrence). This test fails if the frozen path is ever repointed back at the live one."""
    import inspect

    src = inspect.getsource(test_acceptance_q54_s79_frozen_report_reproduces_l322s_effective_n)
    assert "reports" in src
    assert "q54_s79_flow_continuation.json" not in src
    assert Q54_FROZEN_REPORT.endswith("-2026-08-09.json")


def test_acceptance_live_q54_report_if_present_is_a_superset_population():
    """The live report is allowed to MOVE — this test asserts the direction, never the value.

    Committed tape only grows, so a later firing of the sealed probe can only see at least as
    many settled game units as the frozen snapshot did. Asserting the inequality (not the
    number) keeps a real regression visible — a shrinking population would mean tape or
    settlement coverage went backwards — without re-creating the pin L341 just removed. This is
    the one test declared in `scripts/invariants.py::MUTABLE_REPORT_PIN_ALLOWLIST`."""
    import json
    from pathlib import Path

    live = Path(__file__).resolve().parent.parent / "reports" / "q54_s79_flow_continuation.json"
    if not live.is_file():
        pytest.skip("live q54 report not present in this checkout")
    frozen = json.loads(_frozen_q54_report_path().read_text(encoding="utf-8"))
    now = json.loads(live.read_text(encoding="utf-8"))
    if "bootstrap" not in now:
        pytest.skip("live q54 report is in its sealed (pre-gate) state")
    assert now["bootstrap"]["n_units"] >= frozen["bootstrap"]["n_units"]
    assert now["bootstrap"]["n_obs"] >= frozen["bootstrap"]["n_obs"]
    # the frozen comparand's own identity is pinned above; the live one carries the same
    # pre-registration, which is what makes the two comparable at all (L311 seal).
    assert now["preregistration_sha256"] == frozen["preregistration_sha256"]
